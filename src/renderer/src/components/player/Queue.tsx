...
export function Queue(): JSX.Element | null {
  ...
  return (
    <aside className="flex h-full w-72 shrink-0 flex-col border-s border-base-800 bg-base-950">
      <div className="flex h-12 items-center justify-between border-b border-base-800 px-4">
        <p className="text-sm font-semibold text-base-100">{t('queue.title')}</p>
        <IconButton label={t('actions.closeQueue')} size="sm" onClick={toggleQueue}>
          <X size={16} />
        </IconButton>
      </div>
      <div className="flex-1 overflow-y-auto p-2">
        {queue.length === 0 ? (
          <p className="px-2 py-6 text-center text-sm text-base-500">{t('queue.empty')}</p>
        ) : (
          queue.map((item, index) => (
            <button
              ...
              className={clsx(
                'flex w-full min-w-0 items-center gap-2.5 rounded-md px-2 py-1.5 text-left transition-colors hover:bg-base-800/60',
            ...
            >
              ...
              <span className="shrink-0 text-[11px] tabular-nums text-base-500">{formatDuration(item.duration)}</span>
            </button>
          ))
        )}
      </div>
